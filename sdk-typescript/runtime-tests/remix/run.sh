...
PORT=${RUNTIME_TEST_PORT:-3803}
PORT="$PORT" npx remix-serve build/server/index.js >/tmp/remix-runtime.log 2>&1 &
PID=$!
trap "kill -9 $PID 2>/dev/null || true; pkill -9 -f remix-serve 2>/dev/null || true" EXIT
