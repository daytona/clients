...
module DaytonaApiClient
  class SnapshotStateUpdatedWebhook < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DaytonaApiClient::SnapshotStateUpdatedWebhook` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DaytonaApiClient::SnapshotStateUpdatedWebhook`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] old_state Value to be assigned
    def old_state=(old_state)
      if old_state.nil?
        fail ArgumentError, 'old_state cannot be nil'
      end
    ...
    def hash
      [event, timestamp, id, name, organization_id, old_state, new_state, updated_at].hash
    end
