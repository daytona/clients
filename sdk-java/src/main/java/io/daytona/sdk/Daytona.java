...
public class Daytona implements AutoCloseable {
    ...
    private boolean analyticsApiUrlFetched;
    ...
    private String getAnalyticsApiUrl() {
        synchronized (analyticsApiUrlLock) {
            if (!analyticsApiUrlFetched) {
                analyticsApiUrl = ExceptionMapper.callMain(
                        () -> new io.daytona.api.client.api.ConfigApi(apiClient)
                                .configControllerGetConfig().getAnalyticsApiUrl());
                analyticsApiUrlFetched = true;
            }
    ...
    private CreateSandbox baseSandboxBody(io.daytona.sdk.model.CreateSandboxParams params) {
        ...
        if (params.getAutoPauseInterval() != null && params.getAutoPauseInterval() < 0) {
            throw new IllegalArgumentException("autoPauseInterval must be a non-negative integer");
        }
        if (params.getAutoStopInterval() != null && params.getAutoPauseInterval() != null
                && params.getAutoStopInterval() != 0 && params.getAutoPauseInterval() != 0) {
