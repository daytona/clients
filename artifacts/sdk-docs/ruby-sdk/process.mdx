...
- `options` _Hash_ - Process creation options accepted by the process service:
...
- `Daytona:ProcessHandle` - Handle used to supervise the process

**Raises**:

- `Daytona:Sdk:Error` - If the process cannot be started
...
- `wait_timeout_ms` _Integer, nil_ - Maximum time to wait, in milliseconds
- `on_stdout` _Proc, nil_ - Callback invoked with decoded stdout chunks
- `on_stderr` _Proc, nil_ - Callback invoked with decoded stderr chunks
- `options` _Hash_ - Process creation options accepted by {#start}

**Returns**:

- `Daytona:ProcessRunResult` - Collected output and the supervising handle

**Raises**:

- `Daytona:Sdk:Error` - If the process cannot be started, followed, or read
...
- `id` _String_ - Process ID

**Returns**:

- `Daytona:ProcessHandle` - Reconnected process handle

**Raises**:

- `Daytona:Sdk:Error` - If the process does not exist or cannot be retrieved
...
- `state` _String, nil_ - Process state filter
- `kind` _String, nil_ - Process kind filter, such as `exec` or `pty`
- `name` _String, nil_ - Process name filter
- `session_id` _String, nil_ - Owning session ID filter

**Returns**:

- `Array\<DaytonaToolboxApiClient:Process\>` - Matching processes

**Raises**:

- `Daytona:Sdk:Error` - If processes cannot be listed
