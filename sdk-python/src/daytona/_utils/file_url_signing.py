...
SIGNING_KEY_CACHE_TTL_SECONDS = 15
...
def build_signed_file_url(
    ...
    if not toolbox_proxy_url:
        raise DaytonaError(
            "Sandbox toolbox proxy URL is not available. "
            + "Call refresh_data() or fetch the sandbox by ID to load it."
        )
    ...
    return f"{toolbox_proxy_url.rstrip('/')}/{sandbox_id}{operation_path}?{query}"
