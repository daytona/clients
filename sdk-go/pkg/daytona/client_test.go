...
func TestClientGetAndListSuccess(t *testing.T) {
	...
	t.Run("list maps query and yields sandboxes via iterator", func(t *testing.T) {
		...
		require.NotNil(t, collected[0].SandboxClass)
		assert.Equal(t, apiclient.SANDBOXCLASS_LINUX_VM, *collected[0].SandboxClass)
		require.NotNil(t, collected[0].WarmPoolId)
		assert.Equal(t, "wp-abc", *collected[0].WarmPoolId)
		require.NotNil(t, collected[0].GpuType)
		assert.Equal(t, apiclient.GPUTYPE_H100, *collected[0].GpuType)
		require.NotNil(t, collected[0].DesiredState)
		assert.Equal(t, apiclient.SANDBOXDESIREDSTATE_STARTED, *collected[0].DesiredState)
		require.NotNil(t, collected[0].DaemonVersion)
