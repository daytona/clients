...
from daytona_toolbox_api_client_async.exceptions import ApiException as ToolboxApiException
...
class AsyncProcess:
    ...
    async def run(
        ...
        *,
        ...
        handle = AsyncProcessHandle(self, process.id)

        if on_stdout is not None or on_stderr is not None:
            stdout, stderr, timed_out = await self._stream_run_output(handle, on_stdout, on_stderr, wait_timeout_ms)
            result = await self._wait_for_process(process.id, timeout_ms=1 if timed_out else None)
        else:
            result = await self._wait_for_process(process.id, timeout_ms=wait_timeout_ms)
            stdout, stderr = await self._collect_run_output(handle)
    ...
    async def _collect_run_output(self, handle: AsyncProcessHandle) -> tuple[str, str]:
        ...
        for _ in range(10_000):
            page = await handle.logs(cursor=cursor, limit=1000, encoding="base64")
    ...
    @with_instrumentation()
    async def connect(self, id: str) -> AsyncProcessHandle:
    ...
    async def list(
        ...
        *,
        ...
        Example:
            ...
            ```
        """
        processes = await self._invoke_process(
    ...
    async def _create_process(
        ...
        *,
        ...
        return await self._invoke_process(self._api_client.create_process, request=request)

    async def _cleanup_process(self, process_id: str) -> None:
        await self._invoke_process(self._api_client.cleanup_process, id=process_id)

    async def _get_process_record(self, process_id: str) -> ToolboxProcessRecord:
        return await self._invoke_process(self._api_client.get_process, id=process_id)
    ...
    async def _send_process_stdin(self, process_id: str, data: str | bytes) -> None:
        ...
        _ = await self._invoke_process(self._api_client.send_process_stdin, id=process_id, request=request)
    ...
    async def _signal_process(
        ...
        *,
        ...
        _ = await self._invoke_process(self._api_client.signal_process, id=process_id, request=request)
    ...
    async def _resize_process(self, process_id: str, *, cols: int, rows: int) -> None:
        ...
        _ = await self._invoke_process(self._api_client.resize_process, id=process_id, request=request)

    async def _wait_for_process(self, process_id: str, *, timeout_ms: int | None = None) -> ProcessResult:
        return await self._invoke_process(self._api_client.wait_for_process, id=process_id, timeout_ms=timeout_ms)
    ...
    async def _invoke_process(
        self,
        operation: Callable[..., Awaitable[_ProcessResultT]],
        **kwargs: object,
    ) -> _ProcessResultT:
        try:
    ...
    async def _get_legacy_entrypoint_session(self, *, request_timeout: float | None) -> Session:
    ...
    async def _list_legacy_sessions(self, *, request_timeout: float | None) -> list[Session]:
