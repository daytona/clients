...
async def test_async_pty_info_resize_list_and_unknown_id_shape(async_sandbox) -> None:
    ...
    handle = await async_sandbox.process.create_pty_session(pty_id, lambda _data: None, pty_size=PtySize(cols=80, rows=24))
...
async def test_async_pty_connect_echo_and_kill_wait_semantics(async_sandbox) -> None:
    pty_id = unique_name("golden-pty")
    first_handle = await async_sandbox.process.create_pty_session(pty_id, lambda _data: None, pty_size=PtySize(cols=80, rows=24))
