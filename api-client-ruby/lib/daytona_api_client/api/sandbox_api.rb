...
module DaytonaApiClient
  class SandboxApi
    ...
    # @option opts [Float] :min_gpu Minimum GPU
    # @option opts [Float] :max_gpu Maximum GPU
    ...
    def list_sandboxes_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'min_gpu'].nil? && opts[:'min_gpu'] < 0
        fail ArgumentError, 'invalid value for "opts[:"min_gpu"]" when calling SandboxApi.list_sandboxes, must be greater than or equal to 0.'
      end

      if @api_client.config.client_side_validation && !opts[:'max_gpu'].nil? && opts[:'max_gpu'] < 0
        fail ArgumentError, 'invalid value for "opts[:"max_gpu"]" when calling SandboxApi.list_sandboxes, must be greater than or equal to 0.'
      end
      ...
      query_params[:'minGpu'] = opts[:'min_gpu'] if !opts[:'min_gpu'].nil?
      query_params[:'maxGpu'] = opts[:'max_gpu'] if !opts[:'max_gpu'].nil?
