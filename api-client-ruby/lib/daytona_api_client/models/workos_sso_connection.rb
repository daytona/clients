...
module DaytonaApiClient
  class WorkosSsoConnection < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DaytonaApiClient::WorkosSsoConnection` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DaytonaApiClient::WorkosSsoConnection`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] connection_type Value to be assigned
    def connection_type=(connection_type)
      if connection_type.nil?
        fail ArgumentError, 'connection_type cannot be nil'
      end
    ...
    def hash
      [id, name, connection_type, state, domains, created_at, updated_at].hash
    end
