...
```ruby
def initialize(api_key: nil, jwt_token: nil, api_url: nil, organization_id: nil, target: nil, otel_enabled: nil, event_streaming: nil, _experimental: nil)
...
- `event_streaming` _Boolean, nil_ - Enable WebSocket sandbox event streaming.
