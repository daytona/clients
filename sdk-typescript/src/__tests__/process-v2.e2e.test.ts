...
const describeProcessV2E2E = process.env.DAYTONA_V2_E2E === '1' ? describe : describe.skip
...
describeProcessV2E2E('TypeScript SDK Process v2 E2E (opt-in)', () => {
  let daytona: Daytona
  let sandbox: Sandbox

  beforeAll(async () => {
    if (!process.env.DAYTONA_API_KEY) {
      throw new Error('DAYTONA_API_KEY environment variable is required for Process v2 E2E tests')
    }

    daytona = new Daytona()
    sandbox = await daytona.create({
      name: `sdk-ts-process-v2-${Date.now()}`,
      language: 'python',
      labels: { purpose: 'process-v2-e2e' },
    })
  })
  ...
  test('serializes a handle and rehydrates it from a fresh client for full replay', async () => {
    const handle = await sandbox.process.start({
      shellCommand: 'i=1; while [ "$i" -le 20 ]; do printf "line %02d\\n" "$i"; i=$((i+1)); sleep 0.05; done',
    })
    const serialized = handle.toJSON()

    const freshClient = new Daytona()
    try {
      const adopted = await freshClient.processHandleFromJSON(serialized)
  ...
  test('writes stdin to cat and closes stdin cleanly', async () => {
    ...
    await handle.stdin('hello process v2\n')
    ...
    expect(logs).toContain('hello process v2')
  })
})
