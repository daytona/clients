...
func TestE2E(t *testing.T) {
	...
	signedUrlHTTPClient := &http.Client{Timeout: 60 * time.Second}
...
PY`, options.WithExecuteTimeout(10*time.Second))
	...
	t.Run("SignedURL/DownloadURLServesContent", func(t *testing.T) {
		...
		resp, httpErr := signedUrlHTTPClient.Get(url)
	...
	t.Run("SignedURL/UploadURLAcceptsFile", func(t *testing.T) {
		...
		resp, httpErr := signedUrlHTTPClient.Post(url, writer.FormDataContentType(), &buf)
		require.NoError(t, httpErr)
		defer resp.Body.Close()
