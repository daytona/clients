...
async def start(
        *,
        ...
        term: str | None = None,
        keep_logs: ProcessKeepLogsName | None = None) -> AsyncProcessHandle
```
...
async def run(
        *,
        ...
        on_stderr: Callable[[str], None] | None = None) -> ProcessRunOutput
```
...
#### AsyncProcess.connect

```python
@intercept_errors(message_prefix="Failed to connect to process: ")
@with_instrumentation()
async def connect(id: str) -> AsyncProcessHandle
```
...
@with_instrumentation()
async def get(id: str) -> AsyncProcessHandle
```
...
async def list(*,
               ...
               session_id: str | None = None,
               name: str | None = None,
               pid: int | None = None) -> list[AsyncProcessHandle]
```
...
```python
def decode_run_frame(channel: object, data: str,
                     encoding: str | None) -> tuple[str | None, str]
```
