...
async def test_async_process_rehydrate_and_resume(async_daytona_client: AsyncDaytona) -> None:
    sandbox = await async_daytona_client.create(timeout=120)
    try:
        handle = await sandbox.process.start(shell_command='printf "one\\n"; sleep 0.2; printf "two\\n"')
        result = await handle.wait()

        fresh = await async_daytona_client.get(sandbox.id)
        rehydrated = await fresh.process.connect(handle.id)
