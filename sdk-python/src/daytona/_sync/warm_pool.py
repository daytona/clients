...
from daytona_api_client import CreateWarmPool, UpdateWarmPool, WarmPoolsApi
...
class WarmPoolService:
    ...
    def list(self) -> list[WarmPool]:
        ...
        Example:
            ...
            ```
        """
        return [WarmPool.from_dto(pool) for pool in self.__warm_pools_api.list_warm_pools()]

    @with_instrumentation()
    def create(self, snapshot: str, pool: int, target: str | None = None) -> WarmPool:
        ...
        Example:
            ```python
            daytona = Daytona()
            pool = daytona.warm_pool.create("my-snapshot", pool=5)
            print(f"Created warm pool {pool.id} in {pool.target}")
            ```
        """
        return WarmPool.from_dto(
            self.__warm_pools_api.create_warm_pool(CreateWarmPool(snapshot=snapshot, pool=pool, target=target))
        )

    @with_instrumentation()
    def update(self, warm_pool_id: str, pool: int) -> WarmPool:
        ...
        Example:
            ```python
            daytona = Daytona()
            pool = daytona.warm_pool.update("warm-pool-id", pool=10)
            ```
        """
        return WarmPool.from_dto(self.__warm_pools_api.update_warm_pool(warm_pool_id, UpdateWarmPool(pool=pool)))

    @with_instrumentation()
    def delete(self, warm_pool_id: str) -> None:
        """Delete a Warm Pool.
        ...
        Example:
            ```python
            daytona = Daytona()
            daytona.warm_pool.delete("warm-pool-id")
            print("Warm pool deleted")
            ```
        """
        self.__warm_pools_api.delete_warm_pool(warm_pool_id)
