...
import type { ProcessLogEofEvent, ProcessLogEvent, ProcessLogStateEvent, ProcessLogWarningEvent } from './types/Process'
import { PROCESS_KEEP_LOGS, PROCESS_KINDS, PROCESS_SHELL_SELECTORS, PROCESS_STDIN_MODES } from './types/Process'
...
function requireString(payload: JsonRecord, key: string): string {
  ...
  if (typeof value !== 'string') {
    throw new DaytonaError(`Invalid process SSE payload: ${key} must be a string`)
  }
  return value
}
...
function optionalStringArray(payload: JsonRecord, key: string): string[] | undefined {
  ...
  for (const entry of value) {
    if (typeof entry !== 'string') {
      throw new DaytonaError(`Invalid process SSE payload: ${key} must contain only strings`)
    }
    result.push(entry)
  }
  return result
...
function optionalStringRecord(payload: JsonRecord, key: string): { [key: string]: string } | undefined {
  ...
  for (const [entryKey, entryValue] of Object.entries(value)) {
    if (typeof entryValue !== 'string') {
      throw new DaytonaError(`Invalid process SSE payload: ${key}.${entryKey} must be a string`)
    }
...
function parseLiteralUnion<T extends string>(value: unknown, key: string, allowed: readonly T[]): T {
  ...
  if (match === undefined) {
    throw new DaytonaError(`Invalid process SSE payload: ${key} has unsupported value ${value}`)
  }
  return match
}
...
export function parseProcessLogFrame(value: unknown): ProcessLogFrame {
  const payload = expectObject(value, 'Invalid process SSE payload: log frame must be an object')
  ...
  const frame: ProcessLogFrame = {
    ...
    seq: (() => {
      ...
      if (typeof seq !== 'number') {
        throw new DaytonaError('Invalid process SSE payload: seq must be a number')
      }
      return seq
    })(),
...
export function parseProcessRecord(value: unknown): ProcessRecord {
  const payload = expectObject(value, 'Invalid process SSE payload: process must be an object')
...
function parseWarningEvent(value: unknown): ProcessLogWarningEvent {
  const payload = expectObject(value, 'Invalid process SSE payload: warning must be an object')
  return {
    type: 'warning',
    cursor: requireString(payload, 'cursor'),
...
function parseStateEvent(value: unknown): ProcessLogStateEvent {
  const payload = expectObject(value, 'Invalid process SSE payload: state must be an object')
...
function parseEofEvent(value: unknown): ProcessLogEofEvent {
  const payload = expectObject(value, 'Invalid process SSE payload: eof must be an object')
  return {
    type: 'eof',
    cursor: requireString(payload, 'cursor'),
  }
...
export function parseProcessLogEvent(eventName: string, data: string): ProcessLogEvent {
  ...
  switch (eventName) {
    ...
    default:
      throw new DaytonaError(`Unsupported process SSE event: ${eventName}`)
  }
}
