...
import type { ProcessKillOptions, ProcessStartOptions, SerializedProcessHandle } from './types/ProcessV2'
import { RUNTIME, Runtime } from './utils/Runtime'

export const PROCESS_NOT_FOUND_CODE = 'PROCESS_NOT_FOUND'
export const DAEMON_UPGRADE_REQUIRED_CODE = 'DAEMON_UPGRADE_REQUIRED'
...
export async function createFetchResponseError(response: Response): Promise<DaytonaError> {
  ...
  const details =
    typeof payload?.details === 'object' && payload.details !== null
      ...
      : undefined
  const firstAvailableCursor =
    typeof details?.firstAvailableCursor === 'string' ? details.firstAvailableCursor : undefined
  ...
  const error =
    code === 'CURSOR_EXPIRED' && source === SOURCE_DAEMON
      ? new DaytonaCursorExpiredError(message, response.status, headers, code, source, firstAvailableCursor)
      : createDaytonaError(message, response.status, headers, code, source)

  if (stringDetails && Object.keys(stringDetails).length > 0) {
    error.details = stringDetails
  }
  return error
}
...
export function normalizeSerializedHandle(serialized: SerializedProcessHandle): SerializedProcessHandle {
  return {
    sandboxId: normalizeIdentifier(serialized.sandboxId, 'sandboxId'),
    processId: normalizeIdentifier(serialized.processId, 'processId'),
  }
}
...
export function shouldTranslateProcessV2Unsupported(error: unknown): error is DaytonaNotFoundError {
  return error instanceof DaytonaNotFoundError && error.code !== PROCESS_NOT_FOUND_CODE
}
