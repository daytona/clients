...
type socketIOClientConfig struct {
	...
	// Source identifies the SDK (e.g. "sdk-go") and is passed as a query parameter
	Source string
	// SdkVersion is the SDK version and is passed as a query parameter
	SdkVersion string
...
func newSocketIOClient(cfg socketIOClientConfig) (*socketIOClient, error) {
	...
	wsURL, err := buildWebSocketURL(cfg.APIURL, cfg.OrganizationID, cfg.Source, cfg.SdkVersion)
	if err != nil {
...
func buildWebSocketURL(apiURL string, organizationID string, source string, sdkVersion string) (string, error) {
	...
	if sdkVersion != "" {
		query.Set("sdkVersion", sdkVersion)
	}

	socketPath := strings.TrimRight(parsed.Path, "/") + "/socket.io/"
	wsURL := fmt.Sprintf("%s://%s%s?%s", wsScheme, parsed.Host, socketPath, query.Encode())
	return wsURL, nil
}
...
func (c *socketIOClient) handleMessage(msg []byte) {
	...
	case byte(eioClose):
		// Server-initiated close: close the socket so readLoop unblocks immediately
		// and its defer runs onDisconnect, instead of waiting out the read deadline.
		c.mu.Lock()
		c.connected = false
		c.mu.Unlock()
		if c.conn != nil {
...
func (c *socketIOClient) handleSocketIOPacket(data []byte) {
	...
	case byte(sioDisconnect):
		// Server-initiated disconnect: close the socket so readLoop unblocks immediately
		// and its defer runs onDisconnect, letting EventDispatcher reconnect promptly.
		c.mu.Lock()
		c.connected = false
		c.mu.Unlock()
		if c.conn != nil {
	...
	for {
		...
		case <-ticker.C:
			...
			if time.Since(lastActivity) > threshold {
				...
				c.connected = false
				c.mu.Unlock()
				// Close the socket so a readLoop blocked in ReadMessage unblocks
