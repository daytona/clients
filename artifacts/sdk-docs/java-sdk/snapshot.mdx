...
- `imageName` _String_ - source image name or tag
- `sandboxClass` _SandboxClass_ - target sandbox class; `null` for default
- `regionIds` _List\<String\>_ - IDs of the regions where the snapshot will be available; `null` or empty for the organization default region. Duplicates are ignored and the order carries no meaning — the server selects the region that performs the initial build or pull. Requesting more than one region requires the multi-region snapshots feature to be enabled for the organization, is not supported for GPU snapshots, and is only possible between regions that share an internal registry.
...
Creates a snapshot from a declarative `Image`, available in the given regions, with optional build log streaming.
